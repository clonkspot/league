...
{foreach from=$leagues item="league" name="leagues"}<a href="?part=league&method=ranking&league[id]={$league.id}&filter[clan_name][]={$clan.name|escape}">{if $league.icon}<img src="{$league.icon}" title="{$league.name}">{/if}</a> <a href="?part=league&method=ranking&league[id]={$league.id}&filter[clan_name][]={$clan.name|escape}">{$League.name}</a>{if !$smarty.foreach.leagues.last},{/if}{/foreach}
<br>
