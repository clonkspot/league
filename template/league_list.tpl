        ...
        <tr {if !$league.is_current}class="revoked"{/if}>
            ...
            <td>
                {foreach from=$league.scenarios item="scenario" name="scenarios"}
                    <a href="{url part="game" method="list" q="filter[scenario_name][]={$scenario.name|escape}&filter[scenario_id][]={$scenario.id}&sort[col]=settle_rank&sort[dir]=asc"}">{$scenario.name|escape}</a>{if !$smarty.foreach.scenarios.last},{/if}
                {/foreach}
            </td>
        </tr>
    {/foreach}
